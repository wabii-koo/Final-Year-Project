...
export default function DashboardLayout({
      ...
      try {
        ...
        if (data.success && data.data?.notifications) {
          ...
          if (prevNotifs === null) {
            ...
            const lastAckId = parseInt(
              localStorage.getItem(`lastAcknowledgedNotifId_${uid}`) || "0",
            );
            let lastAckTs = parseInt(
              localStorage.getItem(`lastAcknowledgedNotifTs_${uid}`) || "0",
            );

            if (lastAckTs === 0 && lastAckId > 0) {
              // Migration fallback: initialize lastAckTs to the creation time of the last acknowledged ID
              const lastAckNotif = currentNotifs.find(
                (n: any) =>
                  (n.notificationId || n.notification_id || n.id || 0) ===
                  lastAckId,
              );
              if (lastAckNotif) {
                lastAckTs = new Date(
                  lastAckNotif.createdAt || lastAckNotif.sentAt || 0,
                ).getTime();
                if (lastAckTs > 0) {
                  localStorage.setItem(
                    `lastAcknowledgedNotifTs_${uid}`,
                    lastAckTs.toString(),
                  );
                }
              }
            ...
            const unacked = unackedNew.filter(
              (n: any) =>
                String(n.senderId || n.sender_id || "") !== String(uid),
            );

            const isCreatorRole =
              user?.role === "director" || user?.role === "registrar";
            ...
            const newNotif = currentNotifs.find((n: any) => {
              ...
              return !prevNotifs.some(
                (pn: any) =>
                  (pn.notificationId || pn.notification_id || pn.id || 0) ===
                  nid,
              );
            });
            ...
            const updatedNotif = currentNotifs.find((n: any) => {
              ...
              return (
                match &&
                (match.title !== n.title ||
                  (match.content || match.message) !==
                    (n.content || n.message) ||
                  match.priority !== n.priority ||
                  (match.sentAt || match.createdAt) !==
            ...
            const deletedNotif = prevNotifs.find((pn: any) => {
              ...
              return !currentNotifs.some(
                (n: any) =>
                  (n.notificationId || n.notification_id || n.id || 0) === nid,
              );
            });
            ...
            if (newNotif) {
              ...
              if (
                !isCreatorUser &&
                String(updatedNotif.senderId) !== String(uid)
              ) {
                setLiveAlert({
              ...
              if (
                !isCreatorUser &&
                String(deletedNotif.senderId) !== String(uid)
              ) {
                setLiveAlert({
            ...
            if (notifChanged) {
              window.dispatchEvent(
                new CustomEvent("school-notifications-updated"),
              );
            }
            prevNotifsRef.current = currentNotifs;
          }
          ...
          if (notifs.length > 0) {
            const maxId = Math.max(
              ...notifs.map(
                (n: any) => n.notificationId || n.notification_id || n.id || 0,
              ),
            );
            const isViewingNotifications = pathname.startsWith(
              "/dashboard/notifications",
            );
            if (isViewingNotifications) {
              localStorage.setItem(`lastReadNotifId_${uid}`, maxId.toString());
              localStorage.setItem(
                `lastAcknowledgedNotifId_${uid}`,
                maxId.toString(),
              );
              const maxTs = Math.max(
                ...notifs.map((n: any) =>
                  new Date(n.sentAt || n.createdAt || 0).getTime(),
                ),
              );
              if (maxTs > 0) {
                localStorage.setItem(
              ...
              const count = notifs.filter(
                (n: any) =>
                  (n.notificationId || n.notification_id || n.id || 0) > lastId,
              ).length;
        ...
        const eventsList =
          dataEvents.data?.events || dataEvents.data || dataEvents;
        if (Array.isArray(eventsList)) {
          ...
          if (prevEvents === null) {
            ...
            const newEvent = currentEvents.find((e: any) => {
              ...
              return !prevEvents.some(
                (pe: any) => (pe.eventId || pe.event_id || pe.id || 0) === eid,
              );
            });
            ...
            const deletedEvent = prevEvents.find((pe: any) => {
              ...
              return !currentEvents.some(
                (e: any) => (e.eventId || e.event_id || e.id || 0) === eid,
              );
            });

            let eventChanged = false;
            const isCreatorUserEv =
              user?.role === "director" || user?.role === "registrar";

            if (newEvent) {
              if (
                !isCreatorUserEv &&
                String(newEvent.createdBy || newEvent.created_by) !==
                  String(uid)
              ) {
                setLiveAlert({
              ...
              if (
                !isCreatorUserEv &&
                String(updatedEvent.createdBy || updatedEvent.created_by) !==
                  String(uid)
              ) {
                setLiveAlert({
                  title: "Event Updated",
              ...
              if (
                !isCreatorUserEv &&
                String(deletedEvent.createdBy || deletedEvent.created_by) !==
                  String(uid)
              ) {
                setLiveAlert({
          ...
          if (eventsList.length > 0) {
            const maxId = Math.max(
              ...eventsList.map(
                (e: any) => e.eventId || e.event_id || e.id || 0,
              ),
            );
            ...
            if (isViewingEvents) {
              ...
              const lastId = lastIdStr ? parseInt(lastIdStr) : 0;
              const count = eventsList.filter(
                (e: any) => (e.eventId || e.event_id || e.id || 0) > lastId,
              ).length;
  ...
  useEffect(() => {
    if (!user || (user.role !== "homeroom_teacher" && user.role !== "guardian"))
      return;
      ...
      try {
        ...
        if (data.success && data.data?.conversations) {
          const total = data.data.conversations.reduce(
            (sum: number, c: any) => sum + (parseInt(c.unreadCount) || 0),
            0,
          );
  ...
  const menuItems = [
    ...
    },
    {
      ...
      roles: ["director", "teacher", "homeroom_teacher", "guardian"],
    },
    {
      icon: BookOpen,
      label: "Homework",
      href: "/dashboard/homework",
      roles: ["teacher", "homeroom_teacher", "guardian"],
    },
    {
      icon: Users,
      label: "Students",
    ...
    },
    {
      ...
      roles: ["guardian", "teacher", "homeroom_teacher"],
    },
    {
      icon: Settings,
    ...
    },
    {
      ...
      roles: ["guardian", "homeroom_teacher"],
    },
    {
      icon: FileText,
      label: "Report Cards",
      href: "/dashboard/report-cards",
      roles: ["director", "homeroom_teacher", "guardian"],
    },
    {
      icon: ClipboardList,
      label: "Audit Logs",
    ...
    <div className="flex h-screen bg-brand-bg font-sans">
      ...
      >
        ...
        <div className="w-full flex flex-col py-6 px-4 shrink-0">
          <div
            className={`flex items-center justify-between w-full ${sidebarOpen ? "px-1" : "justify-center"}`}
          >
            {sidebarOpen && (
              <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2 duration-300 min-w-0">
                ...
                <span className="font-black text-sm tracking-tighter uppercase text-white truncate">
                  GuardianGate
                </span>
              </div>
            )}
        ...
        {sidebarOpen && user && (
          <div className="px-5 py-4 mx-4 bg-white/10 rounded-2xl border border-white/10 shadow-inner flex items-center gap-3 mb-6 shrink-0 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="w-10 h-10 rounded-xl overflow-hidden shadow shrink-0 flex items-center justify-center bg-white/20">
              <img
                src="/ethiopian-woman.png"
                alt="User Profile"
                className="w-full h-full object-cover"
              />
            </div>
            <div className="min-w-0 flex-1">
              <p className="font-bold text-white truncate text-sm leading-snug">
                {user.fullName}
              </p>
              <p className="text-[10px] text-white/70 font-semibold uppercase tracking-wider leading-none mt-1">
                {user.role?.replace("_", " ")}
              </p>
            </div>
          </div>
            ...
            return (
              ...
              >
                ...
                {count > 0 &&
                  (sidebarOpen ? (
                    <span
                      className={`${isActive ? "bg-brand-primary text-white" : "bg-brand-white text-brand-primary"} text-[10px] font-black px-2 py-0.5 rounded-full ring-2 ring-white/10`}
                    >
                      {count}
                    </span>
                  ) : (
        ...
        <div className="p-4 border-t border-white/10 shrink-0">
          ...
          >
            <LogOut
              size={20}
              className={sidebarOpen ? "text-white" : "mx-auto"}
            />
          ...
          <div className="fixed top-8 right-8 z-[60] max-w-sm animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="bg-white rounded-3xl shadow-2xl border-2 border-brand-primary p-6 flex items-start gap-5">
              ...
              <div className="flex-1">
                <h4 className="font-black text-brand-heading text-base leading-tight">
                  {liveAlert.title}
                </h4>
                ...
                <button
                  onClick={() => {
                    ...
                    const uid =
                      ...
                      "default";
                    const maxId =
                      currentNotifsRef.current.length > 0
                        ? Math.max(
                            ...currentNotifsRef.current.map(
                              (n: any) =>
                                n.notificationId ||
                                n.notification_id ||
                                n.id ||
                    ...
                    const maxTs =
                      currentNotifsRef.current.length > 0
                        ? Math.max(
                            ...currentNotifsRef.current.map((n: any) =>
                              new Date(n.sentAt || n.createdAt || 0).getTime(),
                            ),
                          )
                        : 0;
