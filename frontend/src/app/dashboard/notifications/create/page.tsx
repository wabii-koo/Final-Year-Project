...
import { Bell, AlertCircle, Users, Calendar, Send, ArrowLeft, AlertTriangle } from 'lucide-react'

interface NotificationData {
  title: string
  content: string
  priority: 'normal' | 'emergency'
  recipientGroup: 'all_guardians' | 'all_teachers' | 'specific_class' | 'specific_users'
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        setError(`Cannot connect to backend server. Please make sure it's running on port 3000. Error: ${fetchError.message}`)
        setLoading(false)
        return
      }
      ...
      if (response.ok && data.success) {
        ...
        setError(data.error?.message || data.message || `Failed: ${response.status} ${response.statusText}`)
      }
    } catch (err: any) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            <div className="flex items-center">
              <Link href="/dashboard">
                <button className="p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 mr-4">
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          <div className="mb-6">
            ...
            <p className="text-gray-600">
              Create and send notifications to guardians, teachers, or specific groups.
            </p>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 transform transition-all">
            ...
            <p className="text-gray-600 text-center mb-6">
              You are about to send an <span className="text-red-600 font-bold uppercase">Emergency Alert</span> to <span className="font-bold text-gray-900">{notification.recipientGroup.replace('_', ' ')}</span>. 
              This will bypass normal priority and provide immediate notification.
            </p>
