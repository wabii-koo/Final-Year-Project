...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const mappedNotifications = (Array.isArray(data) ? data : []).map(
          (item: any) => ({
            ...
            time: item.sentAt
              ...
              : item.createdAt
                ...
                : "Unknown time",
            type: (item.priority === "emergency" ? "error" : "info") as
              | "info"
              | "warning"
              | "success"
              | "error",
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this notification globally?"))
      return;
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/notifications/${editingNotif.id}`,
        {
          method: "PUT",
          headers: {
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-bold text-black mb-2">
                        ...
                        {notification.type === "error" && (
                          <span className="text-red-600 ml-2 uppercase text-xs tracking-wider bg-red-100 px-2 py-0.5 rounded">
                            Emergency
                          </span>
                        )}
                      </p>
      ...
      {viewingNotif && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden">
            <div
              className={`px-6 py-4 border-b flex items-center justify-between ${viewingNotif.type === "error" ? "bg-red-50 border-red-100" : "bg-blue-50 border-blue-100"}`}
            >
              <h2
                className={`text-xl font-bold ${viewingNotif.type === "error" ? "text-red-800" : "text-blue-900"}`}
              >
                {viewingNotif.type === "error" && (
                  <AlertTriangle className="inline-block w-5 h-5 mr-2 -mt-1 text-red-600" />
                )}
