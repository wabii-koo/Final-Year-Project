...
export default function RegistrarDashboard() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStats({
          ...
          totalGuardians: (s.totalApproved || 0) + (s.totalPending || 0) + (s.totalRejected || 0) + (s.totalCorrectionRequired || 0) + (s.totalLocked || 0),
          recentActivity: []
        })
      } else {
  ...
  const downloadCSVTemplate = async () => {
    try {
      ...
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'
      const response = await fetch(`${apiUrl}/api/registration/registrar/students/export`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      ...
      <div className="relative mx-auto max-w-7xl p-6 lg:p-8 space-y-10">
        ...
        <header className="bg-brand-white/80 backdrop-blur-md rounded-3xl p-8 shadow-xl shadow-brand-primary/5 border border-brand-100/60 flex flex-col md:flex-row md:items-center justify-between gap-6 overflow-hidden relative hover:border-brand-primary/20 transition-all duration-500">
          <div className="relative z-10">
            ...
            <p className="text-brand-text mt-2 text-lg font-semibold">
              You have <span className="text-brand-primary font-black">{stats.pendingRegistrations}</span> applications awaiting verification.
            </p>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {statCards.map((stat, i) => (
            ...
            >
              {/* Card Accent Top Bar */}
              <div className={`absolute top-0 left-0 right-0 h-1.5 ${stat.borderGrad}`} />
              
              <div className="flex items-center justify-between">
                <div className={`p-3 rounded-2xl ${stat.bgColor} ${stat.color} group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
                  <stat.icon size={24} />
                </div>
              </div>
              <div className="mt-5">
                <p className="text-brand-text font-bold text-[10px] uppercase tracking-widest leading-none">{stat.label}</p>
                <h3 className="text-3xl font-black text-brand-heading mt-2 leading-none">{stat.value}</h3>
                <p className="text-brand-text/60 text-xs font-semibold mt-3 leading-none">{stat.description}</p>
              </div>
            </div>
          ))}
        </section>
            ...
            <div className="space-y-5 flex-1 flex flex-col justify-center">
              ...
              >
                ...
                <div className="text-left">
                  <span className="block font-black text-brand-heading text-sm group-hover:text-brand-primary transition-colors">Download Student Database (CSV)</span>
                  <span className="block text-[10px] text-brand-text/50 font-bold uppercase tracking-wider mt-0.5">Export active records from database</span>
                </div>
              </button>
              ...
              {uploading && (
                <div className="flex items-center gap-3 p-4 bg-brand-bg rounded-2.5xl border border-brand-100/50">
                  ...
                  <span className="text-xs font-black text-brand-heading uppercase tracking-wider">Uploading student catalog...</span>
                </div>
              )}
              ...
              {uploadResult && (
                <div className="p-5 bg-emerald-50/80 backdrop-blur-xs rounded-2.5xl border border-emerald-100 space-y-4 shadow-sm animate-fadeIn">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-black text-emerald-800 uppercase tracking-widest">Catalog Upload Completed</p>
                  </div>
                  <div className="text-xs text-emerald-700 font-bold space-y-1.5 leading-relaxed">
                    <p>Successfully imported <span className="font-extrabold text-emerald-900">{uploadResult.data.successful}</span> new student profiles.</p>
                    {uploadResult.data.duplicates > 0 && (
                      <p className="text-amber-700">Skipped <span className="font-extrabold text-amber-900">{uploadResult.data.duplicates}</span> duplicate records (already exist in database).</p>
                    )}
                    {uploadResult.data.failed > 0 && (
                      <p className="text-red-700">Failed to import <span className="font-extrabold text-red-900">{uploadResult.data.failed}</span> records due to formatting or database errors.</p>
                    )}
                  </div>
                  {uploadResult.data.created?.length > 0 && (
                    <div className="text-[11px] text-emerald-700 bg-white/55 p-3 rounded-xl border border-emerald-100/50">
                      <p className="font-black uppercase tracking-wider text-emerald-800">Added Students:</p>
                      <ul className="list-disc list-inside mt-2 space-y-1 font-semibold max-h-24 overflow-y-auto">
                        {uploadResult.data.created.map((student: any) => (
                          <li key={student.studentId} className="truncate">{student.fullName}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  {uploadResult.data.errors?.length > 0 && (
                    <div className="text-[11px] text-amber-700 bg-amber-50/55 p-3 rounded-xl border border-amber-100/50">
                      <p className="font-black uppercase tracking-wider text-amber-800">Warnings/Errors details:</p>
                      <ul className="list-disc list-inside mt-2 space-y-1 font-semibold max-h-24 overflow-y-auto">
              ...
              {uploadError && (
                <div className="p-4 bg-red-50/85 backdrop-blur-xs rounded-2.5xl border border-red-100 animate-fadeIn">
                  <p className="text-xs font-black text-red-800 uppercase tracking-wider">Catalog Import Failed</p>
                  <p className="text-xs text-red-700 font-semibold mt-1 leading-relaxed">{uploadError}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col h-full justify-between gap-6">
            {stats.pendingRegistrations > 0 ? (
              ...
              >
                <div className="relative z-10">
                  ...
                  <p className="text-white/80 text-sm mt-3 font-semibold leading-relaxed">
                    You have {stats.pendingRegistrations} applications awaiting review that require immediate verification attention.
                  </p>
                </div>
                <ShieldCheck className="absolute -bottom-6 -right-6 text-white/10 group-hover:rotate-45 group-hover:scale-110 transition-all duration-500" size={120} />
              </div>
            ) : (
              <div className="bg-brand-white/85 border border-brand-100/60 rounded-[2.5rem] p-8 shadow-sm flex-1 flex flex-col justify-center items-center text-center">
                ...
                <h4 className="text-lg font-black text-brand-heading">All Caught Up!</h4>
                <p className="text-xs text-brand-text/70 mt-2 font-medium max-w-[200px]">There are no pending guardian registrations to review.</p>
              </div>
            )}
          </div>
        </div>
