...
async function testMessageDeletion() {
  try {
    ...
    // Helpers for fetch requests
    const apiCall = async (url: string, method: string, token: string, body?: any) => {
    ...
    const hasMsgGuardian = guardianMsgs.data.data.messages.some((m: any) => m.messageId === messageId);
    ...
    const hasMsgTeacher = teacherMsgs.data.data.messages.some((m: any) => m.messageId === messageId);
    ...
    const hasMsgGuardianAfter = guardianMsgsAfter.data.data.messages.some((m: any) => m.messageId === messageId);
    ...
    const hasMsgTeacherAfter = teacherMsgsAfter.data.data.messages.some((m: any) => m.messageId === messageId);
    ...
    const hasMsgGuardianAfterBoth = guardianMsgsAfterBoth.data.data.messages.some((m: any) => m.messageId === messageId2);
    ...
    const hasMsgTeacherAfterBoth = teacherMsgsAfterBoth.data.data.messages.some((m: any) => m.messageId === messageId2);
